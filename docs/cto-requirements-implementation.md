...
all 31 cases below against it over real HTTP, prints a pass/fail line per case, and exits
...
Both secrets fail closed: a production build that cannot find them refuses to start rather
than falling back to a default, since a known default would make every signature in the
system forgeable by anyone who has read this repository.
...
4. **Store egress IPs are placeholders.** `stores.ts` contains RFC 5737 documentation
   addresses. Real values must be registered before any pilot or every customer is denied.
5. **The store directory is still client-side mock data.** `MOCK_STORES` in
   `src/lib/mockData.ts` is public information (name, address, opening state), equivalent
   to a maps listing, so it is out of scope for Requirement 2 — but it is not backed by a
   real API yet.
6. **The product catalogue is in-memory.** Deliberate, per the repository-interface design
   above, but it means data resets on deploy and does not survive across instances.
